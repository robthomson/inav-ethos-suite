...
local function setLogging(level, opts)
    ...
    inavadmin.utils.log(_mspPrefix .. "logging set to level=" .. tostring(_mspLogLevel) .. (_mspHexDump and " (hexdump on)" or " (hexdump off)"), "debug")
end

local function getLogging() return {level = _mspLogLevel, hexdump = _mspHexDump, prefix = _mspPrefix} end

local function _log(level, msg) if _mspLogLevel >= level then inavadmin.utils.log(_mspPrefix .. msg, "info") end end
...
local function _hex(buf, maxLen)
    ...
    if #buf > n then inavadmin.utils.log(_mspPrefix .. "(+" .. tostring(#buf - n) .. " more bytes)", "debug") end
end
...
function handlers.v2.processTxQ()
    ...
    for j = i, inavadmin.tasks.msp.protocol.maxTxBufferSize do payload[j] = payload[j] or 0 end
