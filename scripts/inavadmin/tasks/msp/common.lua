...
local function mspProcessTxQ()
    ...
    inavadmin.utils.log("Sending mspTxBuf size " .. tostring(#mspTxBuf) .. " at Idx " .. tostring(mspTxIdx) .. " for cmd: " .. tostring(mspLastReq), "debug")
...
local function mspSendRequest(cmd, payload, version)
    if not cmd or type(payload) ~= "table" then
        inavadmin.utils.log("Invalid command or payload", "debug")
        return nil
    end
    if #mspTxBuf ~= 0 then
        inavadmin.utils.log("Existing mspTxBuf still sending, failed to send cmd: " .. tostring(cmd), "debug")
        return nil
    end
    local useV =
...
local function mspReceivedReply(payload)
    ...
    if ver == 2 then
        ...
        if (crc & 0xFF) ~= (rxcrc & 0xFF) then
            inavadmin.utils.log("MSPv2 CRC mismatch", "debug")
            return nil
        end
        return true
    end
    ...
    while (idx <= inavadmin.tasks.msp.protocol.maxRxBufferSize) and (#mspRxBuf < mspRxSize) do
        ...
        if value then
            ...
            inavadmin.utils.log("Non-numeric value at payload index " .. idx, "debug")
        end
        idx = idx + 1
    end
    ...
    if mspRxCRC ~= payload[idx] and version == 0 then
        inavadmin.utils.log("Payload checksum incorrect, message failed!", "debug")
        return nil
    end
    return true
end
