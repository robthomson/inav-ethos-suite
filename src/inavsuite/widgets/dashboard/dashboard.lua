...
function dashboard.computeOverlayMessage()
    ...
    if not inavsuite.session.isConnectedHigh and state ~= "postflight" then return "@i18n(widgets.dashboard.waiting_for_connection)@" end
...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showstats or inavsuite.preferences.developer.overlaystats then
        ...
        local budgetMs = (inavsuite.performance and inavsuite.performance.budget_ms) or 50
        local tickMs = (inavsuite.performance and inavsuite.performance.tick_ms)
        ...
        local schedRows = {{"LOAD", fmtPct(cpuUsage), "%"}, {"LOAD (100ms window)", fmtPct(inavsuite.performance.cpuload_window100 or 0), "%"}, {"HEADROOM", fmtPct(headroomPct), "%"}, {"LOOP / BUDGET", fmtMS(loopMs) .. " / " .. fmtMS(budgetMs), "ms"}}
...
function dashboard.create()
    ...
    if not dashboard.utils then dashboard.utils = assert(compile("SCRIPTS:/" .. inavsuite.config.baseDir .. "/widgets/dashboard/lib/utils.lua"))() end
    if not dashboard.loaders then dashboard.loaders = assert(compile("SCRIPTS:/" .. inavsuite.config.baseDir .. "/widgets/dashboard/lib/loaders.lua"))() end
...
function dashboard.wakeup(widget)
    ...
    if firstWakeupCustomTheme and inavsuite.session.mcu_id and inavsuite.session.modelPreferences and inavsuite.session.modelPreferences.dashboard then
...
function dashboard.savePreference(key, value)
    if not inavsuite.session.modelPreferences or not inavsuite.session.modelPreferencesFile or not dashboard.currentWidgetPath then return false end
    if not inavsuite.app.guiIsRunning then
