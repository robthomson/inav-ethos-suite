...
local runCreate = true

local gps_lock_prev = false

local colorTable = {
    ['darkmode'] = {backdrop = lcd.RGB(0, 0, 0), background = lcd.RGB(40, 40, 40), foreground = lcd.RGB(255, 255, 255), label = lcd.RGB(200, 200, 200), hd = "widgets/hud/gfx/hd_white.png"},
    ['lightmode'] = {backdrop = lcd.RGB(255, 255, 255), background = lcd.RGB(208, 208, 208), foreground = lcd.RGB(40, 40, 40), label = lcd.RGB(50, 50, 50), hd = "widgets/hud/gfx/hd_black.png"}
}
local colors
...
function widget.create()
    ...
    if not widget.sensors.telemetry then widget.sensors.telemetry = assert(loadfile("widgets/hud/sensors/telemetry.lua"))() end

    if not widget.render.telemetry then widget.render.telemetry = assert(loadfile("widgets/hud/render/telemetry.lua"))() end
    if not widget.render.ah then widget.render.ah = assert(loadfile("widgets/hud/render/ah.lua"))() end
    if not widget.render.gps then widget.render.gps = assert(loadfile("widgets/hud/render/gps.lua"))() end
    if not widget.render.gps_lock then widget.render.gps_lock = assert(loadfile("widgets/hud/render/gps_lock.lua"))() end
    if not widget.render.map then widget.render.map = assert(loadfile("widgets/hud/render/map.lua"))() end
    if not widget.render.flightmode then widget.render.flightmode = assert(loadfile("widgets/hud/render/flightmode.lua"))() end
    if not widget.render.hd then widget.render.hd = assert(loadfile("widgets/hud/render/homedirection.lua"))() end
    if not widget.render.hd_light then widget.render.hd_light = assert(loadfile("widgets/hud/render/homedirection_light.lua"))() end
end
...
function widget.paint()
    ...
    if widget.layout.gps_lock then
        local opts = {images = {red = "widgets/hud/gfx/red.png", orange = "widgets/hud/gfx/orange.png", green = "widgets/hud/gfx/green.png"}, colorbg = colors.background}
...
function widget.wakeup()
    ...
    if widget.render.map then
        local opts = {
            ...
            show_grid = true,
            show_distance = true,
            home_icon = "widgets/hud/gfx/home.png",
            own_icon = "widgets/hud/gfx/arrow.png",
